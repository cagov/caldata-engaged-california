...
with participants as (
    select * from {{ ref('stg_ethelo_la_deliberation_participants.sql') }}
...
select
    ...
    count(distinct case when status = 'Joined' 
        and last_invite_sent < current_date then participant_id end) as num_invited_before_today_and_joined,
