...
lines AS (
  SELECT
    t.filename,--create a session id from the filename:
...
windowed AS (
  SELECT
    session_id, filename, line_no, line,
...
cues AS (
  SELECT
    ...
    COALESCE(TRY_TO_NUMBER(TRIM(prev_line)),
             ROW_NUMBER() OVER (PARTITION BY filename ORDER BY line_no)) AS seq,
...
SELECT
 ...
 -- Split the speaker off the front of the text. In VTT the text usually reads
  ...
  CASE WHEN SPLIT_PART(raw_text, ':', 1) RLIKE '[A-Za-z][A-Za-z0-9 .,''/()&-]{0,60}'
       THEN TRIM(SPLIT_PART(raw_text, ':', 1)) END AS speaker,
...
-- The text itself. If the prefix looked like a speaker, take everything AFTER
  ...
  CASE WHEN SPLIT_PART(raw_text, ':', 1) RLIKE '[A-Za-z][A-Za-z0-9 .,''/()&-]{0,60}'
       ...
       ELSE TRIM(raw_text) END AS text
FROM cues