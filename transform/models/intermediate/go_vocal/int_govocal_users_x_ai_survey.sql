...
select
    ...
    IFF(ARRAY_SIZE(u.gender_array) > 1, 'Multiple', u.gender_array[0] || ' (only)') as gender_category,
    IFF(ARRAY_SIZE(u.race_ethnicity_array) > 1, 'Multiple', u.race_ethnicity_array[0] || ' (only)')
    ...
    s.fields_completed_count,
    u._loaded_at
from non_admin_users as u
