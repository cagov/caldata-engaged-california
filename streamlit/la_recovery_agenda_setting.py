...
*Representative quotes:*
    ...
    with combined_comments as (
        ...
        -- Second source: INT_PARTICIPANT_PERSPECTIVE_AND_PRIORITY
        select
...
# Function to load source comments for selected topics and participants from both tables
@st.cache_data
def load_source_comments(participant_ids, selected_topics):
    # Convert the list of participant IDs to a SQL-friendly string format
    if not participant_ids:
        participant_ids_str = "''"  # Empty string for SQL query if no participants
    else:
    ...
    # Create the SQL query for INT_PARTICIPANT_PERSPECTIVE_AND_PRIORITY
    query_int = f'''
    SELECT
    ...
    try:
        ...
        stg_comments_df = session.sql(query_stg).to_pandas()
        int_comments_df = session.sql(query_int).to_pandas()

        return {
            'discussion_comments': stg_comments_df,
            'participant_perspectives': int_comments_df
        }
    except Exception as e:
        st.error(f"Error executing source comments query: {e}")
        return {
            'discussion_comments': pd.DataFrame(),
            'participant_perspectives': pd.DataFrame()
        }
...
# Updated load_zip_data_wkt function
@st.cache_data
...
# Create tabs
tab1, tab2, tab3, tab4, tab5 = st.tabs(["Participation", "LLM Analysis", "Source Data", "Participant Map", "Demographics"])
