...
TRANSCRIPT_SYSTEM_PROMPT = (
    ...
    "When you reference what someone said, cite the turn index like [turn:42]. To cite "
    "multiple turns for the same point, use separate tags like [turn:12] [turn:45] — never "
    "combine numbers into one bracket like [turn:12, 45], which will not render. "
    "NEVER fabricate or reproduce quotes from memory — always refer to turns by their index. "
    "Format your response in Markdown with headers, bullet points, and bold text. Do not add a "
    "closing remark, disclaimer, or meta-commentary about your methodology or what other "
    "information is available elsewhere — end when the requested content ends."
)

MAP_PROMPT_TRANSCRIPT = (
    "From this transcript excerpt, extract the following. Cite representative turns like "
    "[turn:N] throughout, and be concise — this summary feeds a larger synthesis. Only cite a "
    "turn number that appears in THIS excerpt and that you have re-checked actually contains "
    "the point you're citing it for — never cite from memory or approximate a nearby number.\n"
...
PROTECT_PROMPT = (
    ...
    "Identify each distinct underlying concern participants actually raised, including concerns "
    "voiced by only one participant. Group related specific points under one shared theme when "
    "they serve the same underlying concern (e.g. 'increase public awareness,' 'require AI "
    "ethics training,' and 'fund more research on AI risks' are all facets of one concern about "
    "AI literacy — list them as sub-points under a single theme, not as three separate themes). "
    "Only split into separate top-level themes when the underlying concern is genuinely "
    "different (e.g. ethics/humanity is not the same concern as privacy if participants treated "
    "them separately). Prefer the participants' own words for theme and sub-point labels. If the "
    "discussion does not substantively address this question, say so and summarize what was "
    "discussed instead. For each theme:\n\n"
    "#### [Theme label]\n\n"
    "*Description: 2-3 sentences on what participants want protected and why.*\n\n"
    "- [Specific point raised] — [turn:N]\n"
    "- [Specific point raised] — [turn:N]\n\n"
    "List one bullet per distinct specific point, each with its own citation. Omit the bullet "
    "list if the theme has only one point — the description and citation already cover it. "
    "You may open with a brief one-line lead-in (e.g. 'The following themes were surfaced:') "
    "but do not restate these instructions or describe your methodology."
)
...
GOV_ACTION_PROMPT = (
    ...
    "schools) — but group proposals under one shared theme when they serve the same underlying "
    "goal (e.g. 'increase public awareness,' 'require employee AI training,' and 'fund more AI "
    "research' are all facets of one 'AI education' goal — list them as sub-points under a "
    "single theme, not as three separate themes). Only split into separate top-level themes "
    "when the underlying goal is genuinely different. If the discussion does not substantively "
    "address this question, say so and summarize what was discussed instead. For each:\n\n"
    "#### [Action label]\n\n"
    "*Description: 2-3 sentences on what participants are asking government to do.*\n\n"
    "- [Specific proposal] — [turn:N]\n"
    "- [Specific proposal] — [turn:N]\n\n"
    "List one bullet per distinct specific proposal, each with its own citation. Omit the "
    "bullet list if the theme has only one proposal — the description and citation already "
    "cover it. You may open with a brief one-line lead-in (e.g. 'The following themes were "
    "surfaced:') but do not restate these instructions or describe your methodology."
)

DELIBERATIVE_PROMPT = (
    "Identify the deliberative moments in this discussion — exchanges where the group's "
    "thinking visibly moves through interaction between participants:\n"
    "- Disagreement between participants\n"
    "- Persuasion, or a participant changing their stated view\n"
    "- A proposal being refined, reworded, narrowed, broadened, merged, split, or deleted "
    "through group input\n"
    "- Participants surfacing shared values or clarifying underlying intent through "
    "questions\n\n"
    "Only count a moment if BOTH hold: at least two different speakers engage with each "
    "other's substance (one person elaborating their own idea uninterrupted does not "
    "qualify), and the exchange is about policy content — what a rule should say, who it "
    "should cover, what it should be called, or whether two ideas are the same idea.\n\n"
    "A moment is one continuous portion of the transcript, not a theme assembled from "
    "scattered turns — unlike the other analyses, here you're pulling out the actual back-"
    "and-forth, not a point it supports. If the same disagreement or proposal resurfaces "
    "later in a separate, non-adjacent exchange, treat that as its own additional moment "
    "rather than folding it into the first.\n\n"
    "For each qualifying moment, describe what happened, who was involved (by turn "
    "reference, not name), and what the outcome was. Cite it as a single span covering the "
    "moment start to finish using [turn:N-M] (e.g. [turn:12-18]), not just the turn where it "
    "starts or ends. If the discussion contains no moment meeting these requirements, say so and "
    "summarize what was discussed instead. You may open with a brief one-line lead-in (e.g. "
    "'The following deliberative moments were identified:') but do not restate these "
    "instructions or describe your methodology."
)
...
def build_transcript_synthesis_prompt(chunk_summaries: list[str], user_prompt: str) -> str:
    ...
    return (
        ...
        "Guidelines:\n"
        "- Merge duplicate mentions of the SAME point across excerpts, and group related points "
        "under one shared theme per the analysis question's own instructions — but never merge "
        "genuinely distinct themes or proposals into one; do not structure your response "
        "per-excerpt\n"
        "- Copy [turn:N] citations EXACTLY as they appear in the excerpt summaries — never "
        "renumber, guess, or invent one. Before writing a citation, re-check the excerpt summary "
        "text to confirm that exact turn number was actually given for that point — if you "
        "aren't sure which excerpt a point came from, omit the citation rather than guess. When "
        "merging mentions of the same point, keep all of its original citations rather than "
        "picking just one\n"
        "- If the excerpt summaries do not address the question, say so — do not invent themes\n"
        "- You may open with a brief one-line lead-in, but do not restate these guidelines or "
        "describe what you're about to do — go straight into the output format requested in the "
        "analysis question below\n\n"
